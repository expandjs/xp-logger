...
module.exports = global.XPLogger = new XP.Class('XPLogger', {
     ...
     *   @param {boolean} [options.console = false] Specifies if the logs should be written also in console
    ...
    initialize(options) {
        ...
        this.console = this.options.console || false;
    ...
    error: {
        ...
        value(error, prefix, callback) {
            ...
            // Checking
            if (!error || error.logged) { callback(); return error; }
            ...
            // Logging
            if (this.console) { console.error(error); }
     ...
     */
    console: {
        set(val) { return XP.isDefined(this.console) ? this.console : !!val; }
    },
